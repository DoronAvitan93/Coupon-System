...
const GetCompanyCouponsByPrice: React.FC<Props> = (props: Props) => {
    ...
    const [companiesCoupons, setCompanies] = useState(null)
    ...
    const getCompaniesHandler = async () => {
        try {
            ...
            const response1 = await fetch("https://doron-coupon-web-app.herokuapp.com/CouponApp/findCompanyCouponsByCompanyIdAndMaxPrice/" + companyIdFromLogin + "/" + props.priceState)
            const response2 = response1.clone();
            const responseFromGetACouponsByMaxPrice = await response1.text();


            if (response1.status === 400) { //BAD REQUEST
                setMessageState({ title: "Oops! Something went wrong!", message: responseFromGetACouponsByMaxPrice })
                throw new Error(responseFromGetACouponsByMaxPrice) // throwing error to stop code to continue and making error
            }
