...
const Login: React.FC<Props> = (props: Props) => {
    ...
    const loginHandler = async (event) => {
        try {
            ...
            const response = await fetch("http://192.168.1.136:8080/CouponApp/login/" + selectUserTypeRef.current.value + "/" + selectUserTypeRef.current.value + "/" + emailRef.current.value + "/" + passwordRef.current.value)
